...
$(document).ready(function(){
    ...
    // 글읽기(view) 대화상자에서 삭제버튼 누르면 해달 글(uid) 삭제 진행
...
function updateList(jsonObj){
    ...
    if(jsonObj.status == "OK"){
        ...
        // [페이징 정보 업데이트]
        var pagination = buildPagination(jsonObj.writepage, jsonObj.totalpage, jsonObj.page, jsonObj.pagerows);
            ...
            success : function(data, status){
                if(status == "success"){
                    if(data.status == "OK"){
                        ...
                        window.viewItem = data.data[0]  // 읽어온걸 전역변수에 담았다!!!
